...
- **Security Group** (`statera-postgres-sg`) — a security group for PostgreSQL with a default egress rule allowing all outbound IPv4 traffic, and an ingress rule allowing TCP port 6432 from `postgres_allowed_cidrs`.

## PostgreSQL Resources

The configuration creates a Managed PostgreSQL cluster:

- **Cluster** (`statera-postgresql`) — single-host PRODUCTION cluster.
- **Database** (`statera`) — the application database.
- **User** (`statera`) — the application database user with a randomly generated password.
- **Password** — generated by the `random_password` resource (24 characters, no special symbols).

### Public Access

The PostgreSQL host is created with a public IP address. Inbound access on port 6432 is restricted **only** to the CIDRs listed in `postgres_allowed_cidrs`. **SSL is required** for all public connections.
